...
@property(strong) NSMutableArray *earlyPresenceElements;
...
//static XMPPRoster* xmppRoster = nil;
//
//+(XMPPRoster*) sharedXMPPRoster{
//	@synchronized(xmppRoster) {
//		if(xmppRoster == nil){
//			xmppRoster = [[XMPPRoster alloc] multicastDelegate];
//		}
//	}
//	
//	return xmppRoster;	
...
- (id)initWithStream:(XMPPStream *)stream rosterStorage:(id <XMPPRosterStorage>)storage
{
	if ((self = [super init]))
	{
		multicastDelegate = (MulticastDelegate <XMPPRosterDelegate> *) [[MulticastDelegate alloc] init];
...
- (BOOL)xmppStream:(XMPPStream *)sender didReceiveIQ:(XMPPIQ *)iq
{
	...
	if (query)
	{
		if (![self hasRoster])
		{
            [self performSelectorOnMainThread:@selector(updateRosterWithQuery:)
                                   withObject:query
                                waitUntilDone:YES];
//      [self performSelectorInBackground:@selector(updateRosterWithQuery:) withObject:query];
        ...
        if ([iq requiresResponse]) {
            ...
            NSXMLElement *resultQuery = [query copy] ;
...
//        [self acceptBuddyRequest:[presence from]];
	}
	else
	{
