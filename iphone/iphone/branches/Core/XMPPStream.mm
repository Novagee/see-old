...
- (void)commonInit
{
	multicastDelegate = (MulticastDelegate <XMPPStreamDelegate> *) [[MulticastDelegate alloc] init];
...
- (BOOL)connect:(NSError **)errPtr
{
	...
	if ([hostName length] == 0)
	{
		...
		srvResolver = [RFSRVResolver resolveWithStream:self delegate:self] ;
...
- (void)sendElement:(NSXMLElement *)element withTag:(long)tag
{
    ...
    NSString *elementName = [element name];
    NSString *outgoingStr;
    if ([elementName isEqualToString:@"presence"]) {
        NSString *status = [element attributeStringValueForName:@"from"];
        if (status != nil) {
            outgoingStr = [NSString stringWithFormat:@"<presence><show>%@</show><status>%@</status><priority>5</priority></presence>", status, status];
        } else {
            outgoingStr = [element compactXMLString];
        }
    } else {
        outgoingStr = [element compactXMLString];
    }
...
- (void)handleStreamFeatures
{
	...
	if (f_bind)
	{
		...
		else
		{
			...
			NSXMLElement *bind = [NSXMLElement elementWithName:@"bind" xmlns:@"urn:ietf:params:xml:ns:xmpp-bind"];
            NSXMLElement *resource = [NSXMLElement elementWithName:@"resource"];
            [resource setStringValue:@"seequiPhone"];
			[bind addChild:resource];
...
- (void)handleBinding:(NSXMLElement *)response
{
	...
	if(r_jid)
	{
		...
		myJID = [XMPPJID jidWithString:fullJIDStr] ;
...
+ (NSString *)generateUUID
{
	...
	if (uuid)
	{
        ...
        result = [NSString stringWithString:(NSString *)CFBridgingRelease(uuidStringRef)];
...
- (void)autoAddDelegate:(id)delegate toModulesOfClass:(Class)aClass
{
	...
	if (delegates == nil)
	{
		delegates = [[MulticastDelegate alloc] init] ;
	...
	if((self = [super init]))
	{
		...
		for(i = 0; i < [components count]; i++)
		{
			...
			if(separator.location != NSNotFound)
			{
                ...
                if(value && key) {
                    CFStringTrimWhitespace((__bridge CFMutableStringRef)key);
