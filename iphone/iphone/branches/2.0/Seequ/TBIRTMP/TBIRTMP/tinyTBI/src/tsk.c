...
* A C structure is defined as an object by using @ref TBI_DECLARE_OBJECT macro in its body.<br>
* A pointer to an object definition shall point to a struct @ref TBI_object_def_s.<br>
* @code
...
* @code
* //(Object defnition)
 static const TBI_object_def_t person_def_t = 
 {
 	sizeof(person_t),
 	person_ctor,
 	person_dtor,
...
* @code
// (constructor)
static TBI_object_t* person_ctor(TBI_object_t * self, va_list * app)
{
 	person_t *person = self;
 	if(person){
 		person->name = TBI_strdup(va_arg(*app, const char *));
 	}
 	return self;
 }
* @endcode 
...
* @code
// (destructor)
 static TBI_object_t * person_dtor(TBI_object_t * self)
 { 
 	person_t *person = self;
 	if(person){
 		TBI_FREE(person->name);
		TBI_object_unref(person->girlfriend);
 	}
 	return self;
 }
* @endcode 
...
* @code
// (comparator)
static int person_cmp(const TBI_object_t *_p1, const TBI_object_t *_p2)
 {
	...
	if((ret = TBI_stricmp(p1->name, p2->name))){
		return ret;
	}
	// do they have the same girlfriend?
	if((ret = TBI_object_cmp(p1->girlfriend, p2->girlfriend))){
		return ret;
	}
...
* When an object is created (see below) the counter value is initialized to 1; this is automatically done and you have nothing to do. The counter is incremented by 1 when you call @ref TBI_object_ref() and decremented (by 1) when you call @ref TBI_object_unref().<br>
...
student_t* s = TBI_null;
//....
((person_t*)s)->name = TBI_strdup("bob");
* @endcode
* 
...
person_t* bob = TBI_object_new(&person_def_t, "bob");
// creates bob's girlfriend
bob->girlfriend = TBI_object_new(&person_def_t, "alice");
// deletes bob: will delete both bob and bob's girlfriend field by calling their destructors
TBI_object_unref(bob);
* @endcode
...
* @code
// create a person
#define PERSON_CREATE(name)	TBI_object_new(&person_def_t, (const char*)name)
* @endcode
* 
* As the destructor has fixed parameters, there is a common macro to destroy all kind of well-defined objects. <br>
* TBI_OBJECT_SAFE_FREE() is used to destroy any object. <br>
...
* Threads can be created using @ref TBI_thread_create() and joined using @ref TBI_thread_join().<br>
* You can temporary cease the executing of a thread by calling @ref TBI_thread_sleep().<br>
* @code
#include "tsk.h"
...
void test_threads()
{
	void* tid[1] = {TBI_null}; // thread id
	int arg = 112; // arg to pass to the function
	
	// creates the thread
	TBI_thread_create(&tid[0], MyThreadFunction, &arg);

	// joins the thread
	TBI_thread_join(&(tid[0]));
}
* @endcode
* 
* <h2>7.2 Mutexes</h2>
...
// create the mutext
TBI_mutex_handle_t *mutex = TBI_mutex_create();
	
TBI_mutex_lock(mutex);
// ...portion of code to protect
TBI_mutex_unlock(mutex);

// destroy the mutex
TBI_mutex_destroy(&mutex);
* @endcode
* Mutexes are not well-defined objects; you should use @ref TBI_mutex_destroy instead of TBI_OBJECT_SAFE_FREE() to destroy them.<br>
* 
* <h2>7.3 Thread-Safe Objects</h2>
* 
* Any C Structure could be declared as thread-safe using @ref TBI_DECLARE_SAFEOBJ macro. It’s not mandatory for the object to be well-defined.<br>
* A thread-safe object is initialized using @ref TBI_safeobj_init() and deinitilized using @ref TBI_safeobj_deinit(). To lock and unlock a portion of code which accesses the object you should use @ref TBI_safeobj_lock() and @ref TBI_safeobj_unlock() respectively.<br>
* 
...
TBI_semaphore_handle_t *sem = TBI_semaphore_create();
// (increment the counter)
TBI_semaphore_increment(sem);
// (decrement the counter)
TBI_semaphore_decrement(sem);
// (destoy the semaphore)
TBI_semaphore_destroy(&sem);
* @endcode
* Semaphores are not well-defined objects; you should use @ref TBI_semaphore_destroy instead of TBI_OBJECT_SAFE_FREE() to destroy them.<br>
...
* @ref TBI_condwait_create is used to create a condition variable, @ref TBI_condwait_wait() to wait indefinitely until the condition is signaled and @ref TBI_condwait_timedwait() to wait until the condition is signaled or ms milliseconds have passed.<br>
* @ref TBI_condwait_signal() is used to alert the first waiting thread that the condition is now true and @ref TBI_condwait_broadcast() is used to alert all waiting threads.<br>
* Condition variables are not well-defined objects; you should use @ref TBI_condwait_destroy() instead of TBI_OBJECT_SAFE_FREE() to destroy them.<br>
* 
* <h2>7.6 Runnable</h2>
* A <i>runnable</i> object is a well-defined object and is declared using @ref TBI_DECLARE_RUNNABLE() macro.<br>
* A <i>runnable</i> object must be explicitly started using @ref TBI_runnable_start() and is implicitly stopped when destroyed. You can explicitly stop the object by calling @ref TBI_runnable_stop().<br>
* 
