 ...
 */
#ifndef _TINYTBI_BINARYUTILS_H_
#define _TINYTBI_BINARYUTILS_H_
...
/**@ingroup TBI_binaryutils_group
* Reverse (bit per bit) a 2-byte value.
* @param value The 2-byte value to reverse.
*/
#define TBI_BINARY_REVERSE_2BYTE(value) ((TBI_BitReverseTable256[value & 0xff] << 8) | (TBI_BitReverseTable256[(value >> 8)]))

/**@ingroup TBI_binaryutils_group
* Converts to uint8_t pointer.
*/
#define TBI_TO_U8(buffer) ((uint8_t*)buffer)
/**@ingroup TBI_binaryutils_group
* Gets the value of the fist byte.
*/
#define TBI_BINARY_GET_1BYTE(buffer) *TBI_TO_U8(buffer)// 1-byte

/**@ingroup TBI_binaryutils_group
* Converts from Little to Big endian.
*/
static uint16_t TBI_LSB_2_MSB( void const * buffer )
{
...
/**@ingroup TBI_binaryutils_group
* @def TBI_BINARY_GET_2BYTES
* Gets the first 2-bytes value.
*/
/**@ingroup TBI_binaryutils_group
* @def TBI_BINARY_SET_2BYTES
* Sets the first 2-bytes value.
*/

#if 0 /*BIG_ENDIAN*/// Do not change this (it's for my own tests)
#	define	TBI_BINARY_GET_2BYTES(buffer) *((uint16_t*)buffer)
#	define  TBI_BINARY_SET_2BYTES(buffer, value) *((uint16_t*)buffer)=value
#else
#	define	TBI_BINARY_GET_2BYTES(buffer) TBI_LSB_2_MSB(buffer)
#	define  TBI_BINARY_SET_2BYTES(buffer, value)							\
...
*/
static const unsigned char TBI_BitReverseTable256[] = 
{
