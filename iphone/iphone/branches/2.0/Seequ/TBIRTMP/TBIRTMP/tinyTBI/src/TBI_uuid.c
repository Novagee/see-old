...
/**@defgroup TBI_uuid_group niversally Unique Identifier (UUID version 5) implementation (RFC 4122).
*/

/**@ingroup TBI_uuid_group
*/
int TBI_uuidgenerate(TBI_uuidstring_t *result)
{
	/* From wikipedia
	...
	*/
	TBI_sha1string_t sha1result;
	TBI_istr_t now;
	unsigned i, k;
	static char HEX[] = "0123456789abcdef";

	TBI_itoa(TBI_time_now(), &now);
	TBI_sha1compute(now, sizeof(now), &sha1result);

	/* XOR the SHA-1 result with random numbers. */
	for(i=0; i<(TBI_UUID_DIGEST_SIZE*2); i+=4){
#if 0
