...
#define jb_warn(...)  //printf(__VA_ARGS__) //(warnf ? warnf(__VA_ARGS__) : (void)0)
#define jb_err(...) //printf(__VA_ARGS__) //(errf ? errf(__VA_ARGS__) : (void)0)
#define jb_dbg(...) //printf(__VA_ARGS__) //(dbgf ? dbgf(__VA_ARGS__) : (void)0)
...
jitterbuffer *jb_new() 
{
  ...
  jb_dbg("N");
  jb = TBI_calloc(1, sizeof(jitterbuffer));
  if (!jb) {
