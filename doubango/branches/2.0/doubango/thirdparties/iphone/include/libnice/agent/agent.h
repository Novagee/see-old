 ...
 *
 <example>
   ...
   // Darwin note: Use ref to one of run loops as first argument in nice_agent_new
   ...
   //Darwin note:
   Use nice_agent_set_signals(NiceAgent* agent, CompatCallback* callbacks)
   instead of g_signal_connect to register for callbacks. Please specify
   NULL for callback if you are not interested in event.
...
#include "candidate.h"
#include "debug.h"
//#include "agent/compat/CompatCallback.h"

#define PACKAGE_STRING "libnice-darwin"
 ...
 */
NiceAgent *
nice_agent_new (GMainContextRef ctx, NiceCompatibility compat);

// Destructor for Darwin:
void nice_agent_dispose (NiceAgent *object);

// Darwin does not support it yet. It's quite useless anyway.
/**
...
/**
 * Expose stun server config too.
 */
gboolean nice_agent_set_stun(NiceAgent* agent, const gchar* stun_ip, guint port);
/**
...
#include "compat/CompatCallback.h"
struct CompatCallback;
// Darwin/Compat
void nice_agent_set_signals(NiceAgent* agent, struct CompatCallback* callbacks);
