...
struct _NiceAgent
{
  ...
  GMainContextRef main_context;     /* main context pointer */
  ...
  GSourceRef discovery_timer_source; /* source of discovery timer */
  GSourceRef conncheck_timer_source; /* source of conncheck timer */
  GSourceRef keepalive_timer_source; /* source of keepalive timer */
  ...
  GSourceRef upnp_timer_source;      /* source of upnp timeout timer */
#endif
  gchar *software_attribute;       /* SOFTWARE attribute */
  gboolean reliable;               /* property: reliable */
	

  //de-GNU Compatinbility changes:
  CompatCallback* callbacks; /*We will use just function pointers instead of GNU signals. this is more portable*/
  /* XXX: add pointer to internal data struct for ABI-safe extensions */
    NiceAddress *remote_rtp;
    NiceAddress *local_rtp;
};
...
GSourceRef agent_timeout_add_with_context (NiceAgent *agent, guint interval, GSourceFunc function, gpointer data);
